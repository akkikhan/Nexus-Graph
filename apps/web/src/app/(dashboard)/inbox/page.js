...
import { MessageSquare, GitPullRequest, Clock, AlertCircle, CheckCircle2, Search, Filter, } from "lucide-react";
    ...
    return (<div className="p-8">
            ...
            <div className="flex gap-4 mb-6">
                <div className="flex-1 relative">
                    ...
                    <input type="text" placeholder="Search pull requests..." className="w-full pl-10 pr-4 py-2.5 bg-zinc-900 border border-zinc-800 rounded-lg text-white placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-nexus-500/50"/>
                </div>
        ...
        ].map((stat, i) => (<motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-xl">
            ...
            <div className="space-y-3">
                {mockPRs.map((pr, i) => (<motion.div key={pr.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + i * 0.1 }} className="p-4 bg-zinc-900/50 border border-zinc-800 rounded-xl hover:border-zinc-700 transition-colors cursor-pointer group">
                        <div className="flex items-start gap-4">
                            {/* Risk indicator */}
                            <div className="flex flex-col items-center gap-1">
                                <div className={`w-2 h-full min-h-[60px] rounded-full ${riskColors[pr.riskLevel]}`}/>
                            </div>
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-3 mb-1">
                                    ...
                                    <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full border ${statusColors[pr.status]}`}>
                                        {pr.status.replace("_", " ")}
                                    </span>
                                </div>
                            ...
                            <div className="text-right">
                                <div className={`text-2xl font-bold ${pr.riskScore >= 75
                ? "text-risk-critical"
                : pr.riskScore >= 50
                    ? "text-risk-high"
                    : pr.riskScore >= 25
