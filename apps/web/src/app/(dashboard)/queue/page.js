...
import { Clock, GitMerge, GitPullRequest, CheckCircle, XCircle, Play, Pause, RotateCcw, AlertTriangle, Zap, } from "lucide-react";
    ...
    return (<div className="p-8">
            ...
            <div className="mb-8">
                ...
                <div className="space-y-3">
                    {queueItems.map((item, index) => (<motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className={`p-4 rounded-xl border ${item.status === "running"
                ? "bg-nexus-950/30 border-nexus-500/30"
                : "bg-zinc-900/50 border-zinc-800"}`}>
                            <div className="flex items-center gap-4">
                                {/* Position */}
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${item.status === "running"
                ...
                : "bg-zinc-800 text-zinc-400"}`}>
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-2">
                                        ...
                                        {item.priority === "high" && (<span className="px-2 py-0.5 bg-red-500/20 text-red-400 text-xs rounded-full">
                                                Priority
                                            </span>)}
                                    </div>
                                ...
                                <div className="text-right">
                                    {item.status === "running" ? (<div>
                                            ...
                                            <div className="w-32 h-1.5 bg-zinc-800 rounded-full mt-2 overflow-hidden">
                                                <motion.div className="h-full bg-nexus-500" initial={{ width: 0 }} animate={{ width: `${item.ciProgress}%` }} transition={{ duration: 0.5 }}/>
                                            </div>
                                            <div className="text-xs text-zinc-500 mt-1">
            ...
            <div>
                ...
                <div className="space-y-2">
                    {recentMerges.map((item, index) => (<motion.div key={index} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} className={`p-3 rounded-lg border flex items-center gap-3 ${item.status === "merged"
                ? "bg-green-950/20 border-green-500/20"
                : "bg-red-950/20 border-red-500/20"}`}>
                            {item.status === "merged" ? (<CheckCircle className="w-5 h-5 text-green-500"/>) : (<AlertTriangle className="w-5 h-5 text-red-500"/>)}
                            <div className="flex-1">
                                <span className="text-white">
                                    #{item.pr.number} {item.pr.title}
                                </span>
                            </div>
                            {item.status === "merged" ? (<span className="text-sm text-zinc-500">
                                    Merged {item.mergedAt} ({item.duration})
                                </span>) : (<span className="text-sm text-red-400">
