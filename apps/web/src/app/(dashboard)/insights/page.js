...
import { BarChart3, TrendingUp, Clock, Users, AlertTriangle, Brain, Zap, } from "lucide-react";
    ...
    return (<div className="p-8">
            ...
            <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {aiInsights.map((insight, i) => (<motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className={`p-4 rounded-xl border ${insight.severity === "high"
                    ...
                    : insight.severity === "medium"
                        ...
                        : "bg-zinc-900/50 border-zinc-800"}`}>
                            <div className="flex items-start gap-3">
                                <div className={`p-2 rounded-lg ${insight.severity === "high"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                {/* Team Velocity */}
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
                    <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                    ...
                    <div className="mb-6">
                        ...
                        <div className="h-3 bg-zinc-800 rounded-full overflow-hidden">
                            <div className="h-full bg-gradient-to-r from-nexus-500 to-purple-500" style={{
            width: `${(velocityData.currentSprint.completed / velocityData.currentSprint.committed) * 100}%`,
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        {velocityData.weeklyTrend.map((week) => (<div key={week.week} className="text-center">
                                <div className="h-20 flex items-end justify-center mb-1">
                                    <div className="w-8 bg-gradient-to-t from-nexus-600 to-nexus-400 rounded-t" style={{ height: `${(week.prs / 20) * 100}%` }}/>
                                </div>
                ...
                {/* Code Health */}
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
                    <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-4 mb-6">
                        <div className="relative w-20 h-20">
                            <svg className="w-full h-full -rotate-90">
                                <circle cx="40" cy="40" r="36" stroke="currentColor" strokeWidth="8" fill="none" className="text-zinc-800"/>
                                <circle cx="40" cy="40" r="36" stroke="url(#gradient)" strokeWidth="8" fill="none" strokeDasharray={`${codeHealthData.overallScore * 2.26} 226`} className="text-nexus-500"/>
                                <defs>
                    ...
                    <div className="space-y-3">
                        {Object.entries(codeHealthData.metrics).map(([key, value]) => (<div key={key}>
                                ...
                                <div className="h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                                    <div className={`h-full rounded-full ${value >= 70
                ? "bg-green-500"
                : value >= 50
                    ? "bg-yellow-500"
                    : "bg-red-500"}`} style={{ width: `${value}%` }}/>
                                </div>
                            </div>))}
                    </div>
                </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Top Reviewers */}
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
                    <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                        <Users className="w-5 h-5 text-blue-500"/>
                        Top Reviewers
                    </h2>
                    <div className="space-y-3">
                        {velocityData.topReviewers.map((reviewer, i) => (<div key={reviewer.username} className="flex items-center gap-3 p-3 bg-zinc-800/50 rounded-lg">
                ...
                {/* Bottlenecks */}
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
                    <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                        <AlertTriangle className="w-5 h-5 text-yellow-500"/>
                        Bottlenecks Detected
                    </h2>
                    <div className="space-y-3">
                        {velocityData.bottlenecks.map((bottleneck, i) => (<div key={i} className="p-4 bg-yellow-950/20 border border-yellow-500/20 rounded-lg">
