...
export const riskCommand = new Command("risk")
    ...
    .action(async () => {
        ...
        try {
            ...
            const sensitiveFiles = diffSummary.files?.filter((f) =>
            ...
            const infraFiles = diffSummary.files?.filter((f) =>
                infraPatterns.some((p) => f.file.toLowerCase().includes(p))
            );
            ...
            const testFiles = diffSummary.files?.filter((f) =>
            ...
            const bar =
                levelColor("█".repeat(filledLength)) +
