...
export const splitCommand = new Command("split")
    ...
    .action(async () => {
        ...
        try {
            ...
            const groups: Record<string, typeof diffSummary.files> = {
                migrations: [],
                backend: [],
                frontend: [],
                tests: [],
            ...
            console.log(
                chalk.gray(`Your PR has ${totalLines} lines across ${diffSummary.files?.length || 0} files.`)
            );
            ...
            if (action === "view") {
                ...
                for (const suggestion of suggestions) {
                    ...
                    const groupKey = Object.keys(groups).find((k) => {
                        if (suggestion.name === "Database Changes") return k === "migrations";
                        if (suggestion.name === "Backend Implementation") return k === "backend";
                        if (suggestion.name === "Frontend Implementation") return k === "frontend";
                        if (suggestion.name === "Tests") return k === "tests";
                        if (suggestion.name === "Configuration") return k === "config";
                        return k === "other";
                    });
