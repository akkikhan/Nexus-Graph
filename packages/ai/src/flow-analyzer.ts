...
export class ReviewFlowAnalyzer {
    private sessionCache: Map<string, ReviewerSession> = new Map();
    ...
    analyzeFlowState(session: ReviewerSession): FlowStateAnalysis {
        ...
        let reviewVelocityTrend: "increasing" | "stable" | "decreasing" = "stable";
        ...
        let commentQualityTrend: "improving" | "stable" | "declining" = "stable";
        if (recentReviews.length >= 3) {
            const firstHalfAvgLength =
                recentReviews.slice(0, 2).reduce((s, r) => s + r.avgCommentLength, 0) / 2;
            const secondHalfAvgLength =
                recentReviews.slice(-2).reduce((s, r) => s + r.avgCommentLength, 0) / 2;
        ...
        let currentState: FlowStateAnalysis["currentState"] = "optimal";
        let suggestedAction: FlowStateAnalysis["suggestedAction"] = "continue";

        // Check for rubber stamping
        if (
            avgReviewDuration < FATIGUE_THRESHOLDS.RUBBER_STAMP_THRESHOLD_MINUTES &&
        ...
        if (reviewsThisSession >= FATIGUE_THRESHOLDS.MAX_REVIEWS_BEFORE_BREAK) {
            risks.push(
                `${reviewsThisSession} reviews without a break exceeds recommended limit`
            );
        ...
        if (
            avgCommentLength < FATIGUE_THRESHOLDS.MIN_COMMENT_LENGTH_WORDS &&
            recentReviews.some((r) => r.commentsLeft > 0)
        ) {
            risks.push("Comment quality is declining - shorter than usual");
            if (currentState === "optimal") currentState = "declining";
        }

        if (approvalRate >= FATIGUE_THRESHOLDS.APPROVAL_RATE_CONCERN) {
            risks.push(
                `High approval rate (${Math.round(approvalRate * 100)}%) - may be missing issues`
            );
            if (currentState === "optimal") currentState = "declining";
        }
        ...
        if (suggestedAction === "take_break") {
            ...
            recommendations.push(
                `📊 ${session.username} has reviewed ${reviewsThisSession} PRs this session`
            );
        }
        ...
        if (suggestedAction === "reassign") {
            ...
            recommendations.push(
                `⚠️ ${session.username}'s review quality may be compromised`
            );
        }
    ...
    scoreReviewers(
        ...
        candidates: Array<{
            ...
            currentWorkload: number; // PRs in queue
        ...
        return candidates
            .map((candidate) => {
                ...
                // Expertise match
                const matchingAreas = prContext.files.filter((f) =>
                    candidate.expertiseAreas.some((area) =>
    ...
    private avgDuration(
        reviews: ReviewerSession["reviews"]
    ): number {
        if (reviews.length === 0) return 0;
        return (
            reviews.reduce(
                (sum, r) =>
                    sum + (r.completedAt.getTime() - r.startedAt.getTime()) / 1000 / 60,
                0
            ) / reviews.length
        );
    }
