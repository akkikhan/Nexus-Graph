...
export class SmartTestGenerator {
    ...
    async generateEdgeCaseTests(
        ...
        const analysis = await this.orchestrator.chat(
...
Generate complete test code:`;
    ...
    private detectFramework(): "jest" | "vitest" | "mocha" | "playwright" {
    ...
    private extractCode(response: string, language: string): string {
        const codeBlockRegex = new RegExp(
            `\`\`\`(?:${language}|${language.toLowerCase()}|typescript|javascript)?\\n([\\s\\S]*?)\`\`\``,
            "i"
        );
    ...
    private extractFunctions(diff: string): string[] {
    ...
    private extractFunctionContent(diff: string, functionName: string): string | null {
    ...
    private estimateBranches(code: string): number {
    ...
    private estimateCoverage(testCode: string, sourceCode: string): number {
        const testCount = (testCode.match(/\bit\s*\(|test\s*\(/g) || []).length;
        const functionCount = (
            sourceCode.match(/function\s+\w+|const\s+\w+\s*=\s*(?:async\s+)?\(/g) || []
        ).length;
    ...
    private parseEdgeCases(
        response: string
    ): Array<{ input: string; expected: string; reason: string; priority: string }> {
        try {
