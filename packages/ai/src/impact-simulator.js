...
export class ImpactSimulator {
     ...
     */
    simulate(diffs, options) {
        const requestCount = options?.requestCount || 1000;
        const loadMultiplier = options?.userLoad === "peak"
            ? 2
        ...
        for (const diff of diffs) {
            ...
            for (const [patternName, patternConfig] of Object.entries(PERFORMANCE_PATTERNS)) {
                for (const pattern of patternConfig.patterns) {
                    if (pattern.test(addedLines)) {
                        ...
                        if (patternName === "N_PLUS_ONE") {
                            suggestions.push(`Consider using eager loading or batch queries in ${diff.file}`);
                        }
                        if (patternName === "EXTERNAL_API") {
                            suggestions.push(`Add timeout and retry logic for external API calls in ${diff.file}`);
                        }
                        if (patternName === "BLOCKING_OPERATION") {
                            suggestions.push(`⚠️ Blocking operation detected - consider async alternative in ${diff.file}`);
                        }
                    }
                }
            }
        ...
        const performanceImpact = {
            ...
            throughput: {
                ...
                after: Math.round(this.baselineMetrics.throughput * (1 - latencyDelta / 500)),
            ...
            cpuUsage: {
                ...
                after: Math.min(100, this.baselineMetrics.cpuUsage + Math.round(latencyDelta * 0.05)),
                delta: Math.round(latencyDelta * 0.05),
            },
        };
        // Add performance-based suggestions
        if (performanceImpact.latency.p99.delta > 50) {
            suggestions.unshift("⚠️ Significant P99 latency increase - consider caching or optimization");
        }
        if (performanceImpact.throughput.delta < -100) {
            suggestions.unshift("⚠️ Throughput reduction detected - may impact capacity");
        }
        // Calculate confidence
        const confidence = Math.max(0.5, 0.85 - confidenceReduction - riskFactors.length * 0.02);
        return {
            performanceImpact,
            riskFactors,
            errorPaths,
     ...
     */
    simulateRequests(requests, changes) {
        return requests.map((request) => {
            const affectedChange = changes.find((c) => request.path.toLowerCase().includes(c.file.replace(/\.(ts|js)$/, "")));
