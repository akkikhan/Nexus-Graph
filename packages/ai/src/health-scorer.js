...
export class CodeHealthScorer {
     ...
     */
    calculateMetrics(files, testFiles = []) {
        let totalLines = 0;
        let typedLines = 0;
        ...
        for (const file of files) {
            ...
            complexities.push(((ifMatches?.length || 0) +
        ...
        return {
            ...
            typeSafety: Math.round(Math.max(0, 100 - anyTypeCount * 5)),
            documentationCoverage: Math.round((documentedFunctions / Math.max(1, totalFunctions)) * 100),
     ...
     */
    analyzeImpact(beforeFiles, afterFiles, testFiles = []) {
        ...
        for (const key of Object.keys(before)) {
            ...
            const weight = METRIC_WEIGHTS[key] > 10
                ? "high"
    ...
    describeChange(metric, change, isPositive) {
