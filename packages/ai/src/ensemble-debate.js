...
export class AIEnsembleDebate {
     ...
     */
    async debate(diff, models) {
        const startTime = Date.now();
        const rounds = [];
        // Round 1: Asserter makes initial claims
        const asserterResponse = await this.orchestrator.chat([{ role: "user", content: `Review this code diff and identify issues:\n\n${diff.diff}` }], {
        ...
        const challengerPrompt = DEBATE_PROMPT_CHALLENGER.replace("{assertions}", JSON.stringify(assertions, null, 2));
        ...
        const resolutionResponse = await this.orchestrator.chat([{ role: "user", content: "Resolve this debate and provide final consensus." }], {
        ...
        const consensusComments = (resolution.consensus || []).map((item, index) => ({
     ...
     */
    async quickDebate(diff, models) {
        // Both models review independently
        const [response1, response2] = await Promise.all([
            this.orchestrator.chat([{ role: "user", content: `Review:\n${diff.diff}` }], { provider: models[0], systemPrompt: DEBATE_PROMPT_ASSERTER, maxTokens: 2048 }),
            this.orchestrator.chat([{ role: "user", content: `Review:\n${diff.diff}` }], { provider: models[1], systemPrompt: DEBATE_PROMPT_ASSERTER, maxTokens: 2048 }),
        ]);
        ...
        const consensus = issues1.filter((i1) => issues2.some((i2) => i2.location?.line === i1.location?.line &&
            i2.category === i1.category));
        return consensus.map((item, index) => ({
    ...
    calculateOverallConfidence(consensus) {
        if (consensus.length === 0)
            return 1; // No issues = confident
        const avgConfidence = consensus.reduce((sum, c) => sum + (c.confidence || 0.5), 0) / consensus.length;
        return avgConfidence;
    }
}
//# sourceMappingURL=ensemble-debate.js.map