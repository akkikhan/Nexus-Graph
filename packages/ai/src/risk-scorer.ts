...
import type { RiskScore, RiskFactor, DiffContext } from "./types";
...
interface PRMetrics {
    ...
    authorSuccessRate?: number; // 0-1
    authorFamiliarityScore?: number; // 0-1
...
export class RiskScorer {
    ...
    private analyzePRSize(metrics: PRMetrics): RiskFactor {
    ...
    private analyzeSensitiveFiles(diffs: DiffContext[]): RiskFactor {
        const sensitiveFiles = diffs.filter((d) =>
            SENSITIVE_PATTERNS.some((pattern) => pattern.test(d.file))
        );
    ...
    private analyzeInfrastructureChanges(diffs: DiffContext[]): RiskFactor {
        const infraFiles = diffs.filter((d) =>
            INFRA_PATTERNS.some((pattern) => pattern.test(d.file))
        );
    ...
    private analyzeTestCoverage(metrics: PRMetrics): RiskFactor {
        const testRatio =
    ...
    private analyzeAuthorExperience(metrics: PRMetrics): RiskFactor {
    ...
    private analyzeTimingRisk(metrics: PRMetrics): RiskFactor {
    ...
    private async analyzeWithAI(diffs: DiffContext[]): Promise<RiskFactor[]> {
        const factors: RiskFactor[] = [];
...
If no significant risks found, return: []`,
                ...
                ],
                {
                    provider,
                    systemPrompt:
                        "You are a security and code quality expert. Identify real risks only.",
                    maxTokens: 2048,
                    temperature: 0.1,
                }
            );
    ...
    private generateSuggestions(factors: RiskFactor[]): string[] {
