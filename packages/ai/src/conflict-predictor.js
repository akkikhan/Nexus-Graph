...
export class ConflictPredictor {
     ...
     */
    async predictConflicts(myDiffs, myBranch, openPRs, fileHistories) {
        ...
        const recommendations = this.generateRecommendations(conflictingPRs, maxConflictProbability, safeWindow);
        return {
     ...
     */
    rangesNear(a, b, distance) {
        return (Math.abs(a[0] - b[1]) <= distance ||
            Math.abs(b[0] - a[1]) <= distance);
    }
    /**
     * Calculate safe merge window
     */
    calculateSafeWindow(conflictingPRs, allPRs) {
     ...
     */
    generateRecommendations(conflictingPRs, conflictProbability, safeWindow) {
        ...
        if (conflictProbability > 0.7) {
            recommendations.push(`⚠️ High conflict probability (${Math.round(conflictProbability * 100)}%). Coordinate with ${conflictingPRs[0]?.authorUsername}.`);
            recommendations.push("🤝 Consider pairing to resolve conflicts proactively.");
        }
        else if (conflictProbability > 0.3) {
            recommendations.push(`⚡ Moderate conflict risk. Merge within ${safeWindow.hours} hours to avoid issues.`);
        }
        if (conflictingPRs.length === 1) {
            recommendations.push(`💬 Reach out to @${conflictingPRs[0].authorUsername} to coordinate merge order.`);
        }
        else {
            recommendations.push(`📢 ${conflictingPRs.length} PRs may conflict. Consider a sync meeting.`);
        }
        ...
        if (hottestFile && hottestFile[1] > 2) {
            recommendations.push(`🔥 ${hottestFile[0]} is a hotspot. Consider splitting changes.`);
        }
        return recommendations;
    }
}
//# sourceMappingURL=conflict-predictor.js.map