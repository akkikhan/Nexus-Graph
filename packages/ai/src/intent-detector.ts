...
const INTENT_REVIEW_FOCUS: Record<CodeIntent, string[]> = {
    authentication: [
...
export class CodeIntentDetector {
     ...
     */
    async analyze(diffs: DiffContext[]): Promise<IntentAnalysis> {
        const detectedPatterns: DetectedIntent[] = [];
        const intentCounts: Record<CodeIntent, number> = {} as Record<CodeIntent, number>;
        const techDebtFlags: IntentAnalysis["technicalDebtFlags"] = [];
        ...
        for (const diff of diffs) {
            ...
            for (let i = 0; i < lines.length; i++) {
                const line = lines[i];
                if (!line.startsWith("+")) continue; // Only analyze additions
                ...
                for (const [intent, patterns] of Object.entries(INTENT_PATTERNS)) {
                    for (const pattern of patterns) {
                        if (pattern.test(line)) {
                            intentCounts[intent as CodeIntent] =
                                (intentCounts[intent as CodeIntent] || 0) + 1;

                            // Create or update detected pattern
                            const existing = detectedPatterns.find(
                                (p) =>
                                    p.intent === intent &&
                            ...
                            if (existing) {
                                ...
                                detectedPatterns.push({
                                    ...
                                    implications: [],
                                    suggestedChecks: INTENT_REVIEW_FOCUS[intent as CodeIntent] || [],
                                });
                            }
                        }
                    }
        ...
        const sortedIntents = Object.entries(intentCounts)
            .sort((a, b) => b[1] - a[1])
            .map(([intent]) => intent as CodeIntent);
        ...
        const architecturalImpact = {
            affectsAuth: ["authentication", "authorization", "encryption"].some((i) =>
                sortedIntents.includes(i as CodeIntent)
            ),
            affectsData: ["database_operation", "data_transformation", "caching"].some(
                (i) => sortedIntents.includes(i as CodeIntent)
            ),
            affectsPerformance: ["caching", "optimization", "rate_limiting"].some((i) =>
                sortedIntents.includes(i as CodeIntent)
            ),
            affectsSecurity: [
                "authentication",
                ...
                "encryption",
                "validation",
            ].some((i) => sortedIntents.includes(i as CodeIntent)),
            affectsReliability: [
                "retry_logic",
                "error_handling",
                "monitoring",
